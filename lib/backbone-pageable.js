...
(function (factory) {
  ...
  function finiteInt(val, name) {
    if (!_.isNumber(val) || _.isNaN(val) || !_.isFinite(val) || ~~val !== val) {
      throw new TypeError("`" + name + "` must be a finite integer, but was '" + val + "'");
    }
    return val;
  }
  ...
  var PageableCollection = Backbone.Collection.extend({
    ...
    */
    ...
    initialize: function (models, options) {
      ...
      var mode = options.mode || this.mode || PageableProto.mode;
      ...
      if (this.mode == "client") {
        ...
        if (state.totalRecords == null && models && !_.isEmpty(models)) {
    ...
    */
    _onFullCollectionEvent: function (event, model, collection, options) {
      ...
      if (event == "reset" || event == "sort") {
        options = collection;
        collection = model;
        if (pageableCollection.mode == "client") {
    ...
    */
    _checkState: function (state) {
      ...
      if (state.totalRecords != null && state.pageSize != null &&
          state.currentPage != null && state.firstPage != null) {

        finiteInt(state.totalRecords, "totalRecords");
        finiteInt(state.pageSize, "pageSize");
        finiteInt(state.currentPage, "currentPage");
        finiteInt(state.firstPage, "firstPage");
    ...
    /**
       ...
       and deleted and a fetch is immediately issued for the current page from
       the server. Pass `false` to `options.fetch` to skip fetching.
    ...
    switchMode: function (mode, options) {
      ...
      this.state = this._checkState(_.extend({}, this.state, {mode: mode}));

      if (mode == "server" || mode == "infinite" && this.fullCollection) {
    ...
    getPage: function (index, options) {
      ...
      if (_.isString(index)) {
        switch (index) {
        case "first": index = this.state.firstPage; break;
        case "prev": index = this.state.currentPage - 1; break;
        case "next": index = this.state.currentPage + 1; break;
        case "last": index = this.state.lastPage; break;
        default:
          throw new TypeError('If `index` is a string, it must be one of "first", "prev", "next" or "last"');
        }
      }
      else finiteInt(index, "index");
    ...
    parseLinks: function (resp, xhr) {
      ...
      _.each(linkHeader.split(","), function (linkValue) {
        ...
        var url = linkParts[0].replace(self.URL_TRIM_RE, '');
        var params = linkParts.slice(1);
        _.each(params, function (param) {
          var paramParts = param.split("=");
          var key = paramParts[0].replace(self.PARAM_TRIM_RE, '');
          var value = paramParts[1].replace(self.PARAM_TRIM_RE, '');
    ...
    fetch: function (options) {
      ...
      var i, kvp, k, v, kvps = _.pairs(queryParams);
      for (i = 0; i < kvps.length; i++) {
        kvp = kvps[i], k = kvp[0], v = _.result(kvp, 1);
