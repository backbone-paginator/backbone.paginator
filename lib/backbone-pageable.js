  ...
  var PageableCollection = Backbone.Collection.extend({
    ...
    initialize: function (models, options) {
      ...
      if (state.isClientMode) {
        ...
        if (options.full) this.fullCollection.comparator = comparator;
    ...
    */
    _onPageableCollectionEvent: function (event, model, collection, options) {
      ...
      if (event === "add" || event === "remove") {
        ...
        fullCollection[event](model, _.extend({}, options, {at: fullIndex}));

        if (event === "add") this.pop();
        else this.push(fullCollection.at(pageEnd + 1));
      }
      ...
      if (event === "reset") {
        ...
        if (collection.length !== pageSize) {
          var currentPageModels = this.toArray();
          var fullPageModels = fullCollection.toArray();
          ...
          fullCollection.add(modelsToAdd);
          fullCollection.remove(modelsToRemove);
        }
      }

      var otherModel;

      if (event === "change" && (otherModel = fullCollection.getByCid(model.cid))) {
        options = collection;
        collection = null;
        otherModel.set(options.changes, options);
      }

      if (event === "sync") {
        if ((otherModel = fullCollection.get(model.cid))) {
          otherModel.fetch();
        }
      }
    ...
    _onFullCollectionEvent: function (event, model, collection, options) {
      ...
      if (event === "add" || event === "remove") {
        if (options.index >= pageStart && options.index < pageEnd) {
          ...
          pageableCollection[event](model, _.extend({}, options, {at: pageIndex}));
        }

        if (event === "add") pageableCollection.pop();
        else pageableCollection.push(this.at(pageEnd + 1));
      }

      if (event === "reset") {
        options = collection;
        collection = model;
        pageableCollection.reset(this.toArray().slice(pageStart, pageEnd), options);
      }

      var otherModel;

      if (event === "change" && (otherModel = pageableCollection.getByCid(model.cid))) {
        options = collection;
        collection = null;
        otherModel.set(options.changes, options);
      }
      ...
      if (event === "sync") {
        if ((otherModel = pageableCollection.get(model.cid))) {
          otherModel.fetch();
        }
      }
