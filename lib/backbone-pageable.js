  ...
  var PageableCollection = Backbone.Collection.extend({
    ...
    initialize: function (models, options) {
      ...
      if (state.isClientMode) {
        ...
        // same references
        var pageStart = state.pageSize * (state.firstPage === 0 ?
          state.currentPage :
          state.currentPage - 1);
        var pageEnd = pageStart + state.pageSize;
        var args = [0, models.length].concat(fullCollection.models.slice(pageStart, pageEnd));
        models.splice.apply(models, args);
      }
    ...
    */
    _onPageableCollectionEvent: function (event, model, collection, options) {
      ...
      if (event === "change") {
        otherModel = fullCollection.getByCid ?
          fullCollection.getByCid(model.cid) :
          fullCollection.get(model);
        options = collection;
        collection = null;
    ...
    */
    _onFullCollectionEvent: function (event, model, collection, options) {
      ...
      if (event === "change") {
        otherModel = pageableCollection.getByCid ?
          pageableCollection.getByCid(model.cid) :
          pageableCollection.get(model);
        options = collection;
        collection = null;
        otherModel.set(options.changes, options);
      }
