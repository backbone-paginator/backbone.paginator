...
Backbone.Paginator = (function ( Backbone, _, $ ) {
	...
	Paginator.requestPager = Backbone.Collection.extend({
		...
		requestPreviousPage: function (arg) {
			if ( this.page !== undefined ) {
				this.page -= 1;
				this.pager(arg);
			}
		...
		pager: function (arg) {
			this.fetch({
				success: function (model, response){
					if( arg && _.isFunction(arg.success) ){
						arg.success(model, response);
					}
				},
				error: function (model, response){
					if( arg && _.isFunction(arg.error) ){
						arg.error(model, response);
					}
				}
			});
		}
